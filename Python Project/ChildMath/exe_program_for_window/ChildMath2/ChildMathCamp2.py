...
def WelcomeAndPraise(welcome_flag, score):
    ...
    (ww, hh) = (700 , 700)
    tk.Label(praw, text=pic_txt, font=("楷体", 40),bg="pink", fg="Red",width=30, height=30).pack()
...
def MathTypeChose():
    ...
    tk.Button(master, text="确定", command=SubmitType, font=("华文行楷", 12), fg="blue",bg="yellow", width=6).grid(row=9, column=1, sticky="w")
...
def ProblemWin(problem_list):
    ...
    for sing_prob in arry_problem_list:
        ...
        label_entry_fg = "blue"
        tk.Label(prowin, text =prob_txt, font=label_entry_ft, fg=label_entry_fg, width=16).grid(row=i, column=0, sticky="E",padx=10, pady=10)
...
def MathScoreBoard(answer_list):
    ...
    def ListWrongProb(list_wrong):
        ...
        for prob in answer_list:
            ...
            if list_wrong==1:
                if prob[3] in ["Wrong", "Empty"]:
                    ...
                    l4 = tk.Label(txt, text=tuple4_right_value, font=("Helvetica", 14, "bold"), fg="green", bg="white", justify="left"  , width=16)
                ...
                l1 = tk.Label(txt, text=tuple1_equal_sig  , font=("Helvetica", 14, "bold"), fg="black", bg="white", justify="left"  , width=3 )
                l2 = tk.Label(txt, text=tuple2_child_value, font=("Helvetica", 14, "bold"), fg="black", bg="white", justify="left"  , width=8)
                l3 = tk.Label(txt, text=tuple3_wrong_right, font=("Helvetica", 14, "bold"), fg="red"  , bg="white", justify="left"  , width=10)
                l4 = tk.Label(txt, text=tuple4_right_value, font=("Helvetica", 14, "bold"), fg="green", bg="white", justify="left"  , width=16)
                txt.window_create("insert", window=l0)
                txt.window_create("insert", window=l1)
                txt.window_create("insert", window=l2)
        ...
        tk.Button(lbl, text="保存此结果", font=("time", 14), command=SaveAnswerReslut, width=10).grid(row=1, column=0, pady=10)
        tk.Button(lbl, text="退出此界面", font=("time", 14), command=lbl.destroy , width=10).grid(row=1, column=1, pady=10)
    ...
    tk.Button(score_win, text="退出数学训练", font=("华文行楷",20), fg="black",bg="brown" , command=ExitProgram             ).pack(padx=10,pady=10)
