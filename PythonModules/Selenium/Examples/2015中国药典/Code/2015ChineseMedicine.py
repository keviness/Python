...
outputPath = '/Users/kevin/Desktop/program files/python/PythonModules/Selenium/Examples/2015中国药典/Result/'
...
def CrawlHerbInfo(bor, url):
    # 向搜索框中录入关键词
    i = 1
    bor.get(url+str(i)+'&bookId=1')
    while True:
        table = bor.find_element_by_xpath('//*[@id="content_text"]')
        table_text = table.get_attribute('innerText')
        #table_html = table.get_attribute('innerHTML')
        #print('table_text:\n', table_text)
        currentHerbName = bor.find_element_by_xpath('//*[@id="article_info"]/h1').get_attribute('innerText')
        #print('currentHerbName:\n', currentHerbName)
        writeFileToText(table_text, str(i)+'_'+currentHerbName)
    
        if currentHerbName != '麝香':
            bor.execute_script('window.location=\"'+url+str(i)+'&bookId=1'+'\"')
            i += 1
        else: break
...
# 对指定的url发起请求
    url = 'http://www.yaobw.cn/yaobw/book.do?flag=showyao&cid=1&cid2=1&id='
    #url = 'http://www.yaobw.cn/yaobw/book.do?flag=showyao&cid=1&cid2=1&id=618&bookId=1'
